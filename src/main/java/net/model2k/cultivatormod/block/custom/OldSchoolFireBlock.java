...
public class OldSchoolFireBlock extends FireBlock {
    ...
    public void tick(@NotNull BlockState state, @NotNull ServerLevel level, @NotNull BlockPos pos, @NotNull RandomSource random) {
        ...
        spreadFireAround(level, pos, random); // Higher chance = more aggressive
        for (int i = 0; i < 3; i++) {
            level.scheduleTick(pos, this, i + 1);
        }
    }
    private void spreadFireAround(ServerLevel level, BlockPos pos, RandomSource random) {
        for (Direction dir : Direction.values()) {
            BlockPos targetPos = pos.relative(dir);
            if (canBurn(level.getLevel(), targetPos)) {
                if (random.nextInt(100) < Integer.MAX_VALUE) {
