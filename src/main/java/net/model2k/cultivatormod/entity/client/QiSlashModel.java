...
public class QiSlashModel <T extends QiSlashEntity> extends HierarchicalModel<QiSlashEntity> {
    public static final ModelLayerLocation LAYER_LOCATION = new ModelLayerLocation(ResourceLocation.fromNamespaceAndPath("cultivatormod", "qislash"), "main");
    ...
    @Override
    public void setupAnim(@NotNull QiSlashEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.QiSlash.yRot = (Math.random() > 0.5) ? 45.0f : -45.0f;
    }
    @Override
